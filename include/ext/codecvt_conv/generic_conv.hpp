	...
	namespace codecvt_convert
	{
		namespace internal
		{
			namespace detail
			{
				...
				>
				convert(Action action, Context & ctx, const InRange & in, OutRange & out)
				{
			...
			template <class InRange, class OutRange, class ... CvtTypes>
			std::pair<
				typename boost::range_iterator<const InRange>::type,
				typename boost::range_iterator<OutRange>::type
			>
			to_bytes(const std::codecvt<CvtTypes...> & cvt, const InRange & in, OutRange & out)
			{
				const bool resizable = ext::has_resize_method<OutRange>::value;
				...
				typedef typename make_to_bytes_context<std::codecvt<CvtTypes...>>::type ctx_type;
			...
			std::pair<
				...
				typename boost::range_iterator<OutRange>::type
			>
			from_bytes(const std::codecvt<CvtTypes...> & cvt, const InRange & in, OutRange & out)
			{
				const bool resizable = ext::has_resize_method<OutRange>::value;
				using namespace detail;
				if (boost::empty(in)) {
				...
				typedef typename make_from_bytes_context<std::codecvt<CvtTypes...>>::type ctx_type;
		...
		to_bytes(const std::codecvt<CvtTypes...> & cvt, const InRange & in, OutRange & out)
		{
			typedef typename std::codecvt<CvtTypes...>::intern_type intern_type;
			typedef typename std::codecvt<CvtTypes...>::extern_type extern_type;
		...
		std::pair<
			...
			typename boost::range_iterator<OutRange>::type
		>
		from_bytes(const std::codecvt<CvtTypes...> & cvt, InRange const & in, OutRange & out)
		{
			typedef typename std::codecvt<CvtTypes...>::intern_type intern_type;
			typedef typename std::codecvt<CvtTypes...>::extern_type extern_type;
		...
		template <class InRange, class ... CvtTypes>
		auto from_bytes(const std::codecvt<CvtTypes...> & cvt, const InRange & in)
		...
		template <class InRange, class ... CvtTypes>
		auto to_bytes(const std::codecvt<CvtTypes...> & cvt, InRange const & in)
