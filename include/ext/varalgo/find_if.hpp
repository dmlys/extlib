...
namespace varalgo{
	...
	struct find_if_visitor :
		boost::static_visitor<InputIterator>
	{
		InputIterator first, last;

		find_if_visitor(InputIterator first, InputIterator last)
	...
	inline InputIterator
		find_if(InputIterator first, InputIterator last, const boost::variant<VaraintTypes...> & pred)
	{
		return boost::apply_visitor(
			find_if_visitor<InputIterator> {first, last},
			pred);
	}
	
	template <class InputIterator, class Pred>
	inline InputIterator
		find_if(InputIterator first, InputIterator last, Pred pred)
	{
		return std::find_if(first, last, pred);
	}
	...
	inline typename boost::range_iterator<const SinglePassRange>::type
		find_if(const SinglePassRange & rng, const Pred & pred)
	{
		return varalgo::find_if(boost::begin(rng), boost::end(rng), pred);
	}

	template <class SinglePassRange, class Pred>
	inline typename boost::range_iterator<SinglePassRange>::type
		find_if(SinglePassRange & rng, const Pred & pred)
	{
		return varalgo::find_if(boost::begin(rng), boost::end(rng), pred);
	}
		...
		find_if(const SinglePassRange & rng, const Pred & pred)
	{
		return boost::range_return<const SinglePassRange, re>::pack(
			varalgo::find_if(boost::begin(rng), boost::end(rng), pred),
			rng);
	}
