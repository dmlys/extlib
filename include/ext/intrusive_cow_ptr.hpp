...
namespace ext
{
	/// smart pointer for intrusive copy-on-write(cow) types
	/// somewhat similar to boost::intrusive_ptr.
	/// 
	/// see also intrusive_cow_atomic_counter, intrusive_cow_plain_counter helper classes.
	/// 
	...
	private:
		value_type * defval() noexcept { return intrusive_ptr_default(static_cast<value_type *>(nullptr)); }

	public:
		auto use_count() const noexcept { return intrusive_ptr_use_count(m_ptr); }
		auto addref()    const noexcept { return intrusive_ptr_add_ref(m_ptr);  }
		auto release()         noexcept { return intrusive_ptr_release(m_ptr); }
	...
	/// intrusive_cow_atomic_counter implements atomic thread safe reference counter 
	/// for a derived user's class that is intended to be used with intrusive_cow_ptr.
	/// For complex hierarchies do not forget virtual destructor and probably virtual clone function
	template <class Derived>
	class intrusive_cow_atomic_counter
	{
	protected:
		std::atomic<unsigned> m_refs = {0};

	public:
		intrusive_cow_atomic_counter() = default;
		intrusive_cow_atomic_counter(intrusive_cow_atomic_counter &&) = default;
		intrusive_cow_atomic_counter(const intrusive_cow_atomic_counter &) noexcept {};

		intrusive_cow_atomic_counter & operator =(intrusive_cow_atomic_counter &&) = default;
		intrusive_cow_atomic_counter & operator =(const intrusive_cow_atomic_counter &) noexcept { return *this; }


		template <class Type> friend           void intrusive_ptr_add_ref(intrusive_cow_atomic_counter<Type> * ptr) noexcept;
		template <class Type> friend           void intrusive_ptr_release(intrusive_cow_atomic_counter<Type> * ptr) noexcept;
		template <class Type> friend       unsigned intrusive_ptr_use_count(const intrusive_cow_atomic_counter<Type> * ptr) noexcept;
		template <class Type> friend std::nullptr_t intrusive_ptr_default(const intrusive_cow_atomic_counter<Type> * ptr) noexcept;		
		
		template <class Type, class DestType>
		friend void intrusive_ptr_clone(const intrusive_cow_atomic_counter<Type> * ptr, DestType * & dest);
	};

	template <class Type>
	inline void intrusive_ptr_add_ref(intrusive_cow_atomic_counter<Type> * ptr) noexcept
	{
		if (ptr) ptr->m_refs.fetch_add(1, std::memory_order_relaxed);
	}

	template <class Type>
	inline void intrusive_ptr_release(intrusive_cow_atomic_counter<Type> * ptr) noexcept
	{ 
	...
	template <class Type>
	inline unsigned intrusive_ptr_use_count(const intrusive_cow_atomic_counter<Type> * ptr) noexcept
	{
		return ptr ? ptr->m_refs.load(std::memory_order_relaxed) : 0;
	}

	template <class Type>
	std::nullptr_t intrusive_ptr_default(const intrusive_cow_atomic_counter<Type> *) noexcept
	{
		return nullptr;
	}

	template <class Type, class DestType>
	void intrusive_ptr_clone(const intrusive_cow_atomic_counter<Type> * ptr, DestType * & dest)
	{
		assert(ptr);
	...
	/// intrusive_cow_plain_counter implements thread unsafe simple reference counter
	/// for a derived user's class that is intended to be used with intrusive_cow_ptr.
	/// For complex hierarchies do not forget virtual destructor and probably virtual clone function
	template <class Derived>
	class intrusive_cow_plain_counter
	{
	protected:
		unsigned m_refs = 0;

	public:
		intrusive_cow_plain_counter() = default;
		intrusive_cow_plain_counter(intrusive_cow_plain_counter &&) = default;
		intrusive_cow_plain_counter(const intrusive_cow_plain_counter &) noexcept {};

		intrusive_cow_plain_counter & operator =(intrusive_cow_plain_counter &&) = default;
		intrusive_cow_plain_counter & operator =(const intrusive_cow_plain_counter &) noexcept { return *this; }


		template <class Type> friend           void intrusive_ptr_add_ref(intrusive_cow_plain_counter<Type> * ptr) noexcept;
		template <class Type> friend           void intrusive_ptr_release(intrusive_cow_plain_counter<Type> * ptr) noexcept;
		template <class Type> friend       unsigned intrusive_ptr_use_count(const intrusive_cow_plain_counter<Type> * ptr) noexcept;
		template <class Type> friend std::nullptr_t intrusive_ptr_default(const intrusive_cow_plain_counter<Type> * ptr) noexcept;
		
		template <class Type, class DestType>
		friend void intrusive_ptr_clone(const intrusive_cow_plain_counter<Type> * ptr, DestType * & dest);
	};

	template <class Type>
	inline void intrusive_ptr_add_ref(intrusive_cow_plain_counter<Type> * ptr) noexcept
	{
		if (ptr) ++ptr->m_refs;
	}

	template <class Type>
	inline void intrusive_ptr_release(intrusive_cow_plain_counter<Type> * ptr) noexcept
	{
		if (ptr && --ptr->m_refs == 0)
			delete static_cast<Type *>(ptr);
	}
	...
	template <class Type>
	inline unsigned intrusive_ptr_use_count(const intrusive_cow_plain_counter<Type> * ptr) noexcept
	{
		return ptr ? ptr->m_refs : 0;
	}

	template <class Type>
	inline std::nullptr_t intrusive_ptr_default(const intrusive_cow_plain_counter<Type> *) noexcept
	{
		return nullptr;
	}

	template <class Type, class DestType> 
	void intrusive_ptr_clone(const intrusive_cow_plain_counter<Type> * ptr, DestType * & dest)	
	{
		assert(ptr);
		dest = new Type(*static_cast<const Type *>(ptr));
		dest->m_refs = 1u;
