...
namespace ext::openssl
{
	...
	struct openssl_deleter  { void operator()(void * ptr)        const noexcept; };
	...
	template <class Type>
	using openssl_uptr  = std::unique_ptr<Type, openssl_deleter>;
	...
	using evp_md_ctx_uptr     = std::unique_ptr<::EVP_MD_CTX, evp_md_ctx_deleter>;
